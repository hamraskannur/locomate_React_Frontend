...
import {  postReplayComment} from "../../../Api/userApi/postRequest";
import { useSelector } from "react-redux";
import ReplayComment from "./ReplayComment";
import InputEmoji from "react-input-emoji";
import { useNavigate } from "react-router-dom";
import { likeShortsMainComment, shortsReplayComment } from "../../../Api/userApi/videoRequest";
...
const Comment = ({ comment }) => {
  ...
  const [AllReplayComment, setAllReplayComment] = useState(comment?.replayComment);
  ...
  const likeMainLike = async (commentId) => {
    const response = await likeShortsMainComment({ commentId });
    if (response.success) {
      if (like) {
              ...
              <div className="flex mt-2 gap-3 ">
                <div className="border-none ml-0 z-50 grow rounded-full">
                  <InputEmoji
                    ...
                    cleanOnEnter
                    onEnter={handleShortsReplayComment}
                    placeholder="Type "
                  />
                </div>
                <img
                  onClick={handleShortsReplayComment}
